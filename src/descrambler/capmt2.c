...
capmt_pid_add(capmt_t *capmt, int adapter, int pid, mpegts_service_t *s)
{
  ...
  for (i = 0; i < MAX_PIDS; i++) {
    ...
    if (t->pid == pid && t->ecm == ecm) {
      t->pid_refs++;
      tvhtrace(LS_CAPMT2, "%s: adding pid %d adapter %d - reusing (%d)",
  ...
  if (o) {
    ...
    tvhtrace(LS_CAPMT2, "%s: adding pid %d adapter %d, tuner %p, mmi %p, mux %p", capmt_name(capmt), pid, adapter, ca->ca_tuner, mmi, mux);
    if (mux) {
...
capmt_pid_flush_adapter(capmt_t *capmt, int adapter)
{
  ...
  tvhtrace(LS_CAPMT2, "%s: pid flush for adapter %d", capmt_name(capmt), adapter);
...
capmt_connect(capmt_t *capmt, int i)
{
  ...
  if (fd >= 0) {
    tvhdebug(LS_CAPMT2, "%s: Created socket %d", capmt_name(capmt), fd);
...
capmt_socket_close(capmt_t *capmt, int sock_idx)
{
  ...
  if (fd < 0)
    return;
  tvhtrace(LS_CAPMT2, "%s: socket close %d", capmt_name(capmt), fd);
...
capmt_write_msg(capmt_t *capmt, int adapter, int sid, const uint8_t *buf, size_t len)
{
  ...
  if (!capmt_oscam_so_wrapper(capmt)) {
    ...
    if (capmt->capmt_oscam == CAPMT_OSCAM_OLD) {
      ...
      for (i = 0; i < MAX_SOCKETS; i++)
        tvhdebug(LS_CAPMT2, "%s: %s: SOCKETS TABLE DUMP [%d]: sid=%d socket=%d", capmt_name(capmt), __FUNCTION__, i, capmt->sids[i], capmt->capmt_sock[i]);
      if (sid == 0) {
        tvhdebug(LS_CAPMT2, "%s: %s: got empty SID - returning from function", capmt_name(capmt), __FUNCTION__);
        return -1;
      }
      ...
      if (found)
        tvhdebug(LS_CAPMT2, "%s: %s: found sid, reusing socket, i=%d", capmt_name(capmt), __FUNCTION__, i);
      ...
      if (i == MAX_SOCKETS) {
        tvhdebug(LS_CAPMT2, "%s: %s: no free space for new SID!!!", capmt_name(capmt), __FUNCTION__);
        return -1;
      } else {
        capmt->sids[i] = sid;
        tvhdebug(LS_CAPMT2, "%s: %s: added: i=%d", capmt_name(capmt), __FUNCTION__, i);
      }
    }
#endif
  ...
  if (fd <= 0) {
    tvhtrace(LS_CAPMT2, "%s: Unable to send message for sid %i", capmt_name(capmt), sid);
    return -1;
  }

  tvhtrace(LS_CAPMT2, "%s: Sending message to socket %i (sid %i)", capmt_name(capmt), fd, sid);
  tvhlog_hexdump(LS_CAPMT2, buf, len);

  res = send(fd, buf, len, MSG_DONTWAIT);
  if (res < len) {
#if ENABLE_ANDROID
    tvhdebug(LS_CAPMT2, "%s: Message send failed to socket %i (%li)", capmt_name(capmt), fd, (long int)res); // Android bug, ssize_t is long int
#else
    tvhdebug(LS_CAPMT2, "%s: Message send failed to socket %i (%zi)", capmt_name(capmt), fd, res);
#endif
    if (!capmt_oscam_so_wrapper(capmt)) {
...
capmt_send_stop(capmt_service_t *t)
{
  ...
  if (capmt->capmt_oscam == CAPMT_OSCAM_OLD) {
    ...
    if (i == MAX_SOCKETS) {
      tvhdebug(LS_CAPMT2, "%s: %s: socket to close not found", capmt_name(capmt), __FUNCTION__);
      return;
    }

    // closing socket (oscam handle this as event and stop decrypting)
    tvhdebug(LS_CAPMT2, "%s: %s: closing socket i=%d, socket_fd=%d", capmt_name(capmt), __FUNCTION__, i, capmt->capmt_sock[i]);
    capmt->sids[i] = 0;
    capmt->adps[i] = 0;
...
capmt_set_filter(capmt_t *capmt, int adapter, sbuf_t *sb, int offset)
{
  ...
  tvhtrace(LS_CAPMT2, "%s: setting filter: adapter=%d, demux=%d, filter=%d, pid=%d",
...
cont:
  ...
  tvhlog_hexdump(LS_CAPMT2, filter->filter.filter, DMX_FILTER_SIZE);
  tvhlog_hexdump(LS_CAPMT2, filter->filter.mask, DMX_FILTER_SIZE);
  tvhlog_hexdump(LS_CAPMT2, filter->filter.mode, DMX_FILTER_SIZE);
...
capmt_stop_filter(capmt_t *capmt, int adapter, sbuf_t *sb, int offset)
{
  ...
  tvhtrace(LS_CAPMT2, "%s: stopping filter: adapter=%d, demux=%d, filter=%d, pid=%d",
           capmt_name(capmt), adapter, demux_index, filter_index, pid);
  if (adapter >= MAX_CA ||
      demux_index >= MAX_INDEX ||
      filter_index >= MAX_FILTER)
...
capmt_send_key(capmt_t *capmt)
{
  ...
  case CA_ALGO_AES:
    if (cai->cipher_mode == CA_MODE_ECB) {
      ...
      tvherror(LS_CAPMT2, "uknown cipher mode %d", cai->cipher_mode);
      return;
    }
    break;
  default:
    tvherror(LS_CAPMT2, "unknown crypto algorithm %d (mode %d)", cai->algo, cai->cipher_mode);
    return;
  }
...
capmt_analyze_cmd(capmt_t *capmt, uint32_t cmd, int adapter, sbuf_t *sb, int offset)
{
  if (cmd == CA_SET_PID_) {
    ...
    tvhdebug(LS_CAPMT2, "%s: CA_SET_PID adapter %d index %d pid %d (0x%04x)", capmt_name(capmt), adapter, index, pid, pid);
    if (index > 0x100 && index < 0x200 && (index & 0xff) < MAX_INDEX) {
      index &= 0xff;
      if (capmt->capmt_cwmode != CAPMT_CWMODE_OE20) {
        tvhwarn(LS_CAPMT2, "Autoswitch to Extended DES (OE 2.0) CW Mode");
    ...
    if (adapter < MAX_CA && index >= 0 && index < MAX_INDEX) {
      ...
      tvherror(LS_CAPMT2, "%s: Invalid index %d in CA_SET_PID (%d) for adapter %d", capmt_name(capmt), index, MAX_INDEX, adapter);
    }
    ...
    tvhdebug(LS_CAPMT2, "%s: CA_SET_DESCR adapter %d par %d idx %d %02x%02x%02x%02x%02x%02x%02x%02x",
             capmt_name(capmt), adapter, parity, index,
             cw[0], cw[1], cw[2], cw[3], cw[4], cw[5], cw[6], cw[7]);
    if (index < 0)   // skipping removal request
      return;
    if (adapter >= MAX_CA || index >= MAX_INDEX) {
      tvherror(LS_CAPMT2, "%s: Invalid adapter %d or index %d", capmt_name(capmt), adapter, index);
      return;
    }
    if (parity > 1) {
      tvherror(LS_CAPMT2, "%s: Invalid parity %d in CA_SET_DESCR for adapter%d", capmt_name(capmt), parity, adapter);
      return;
    }
    ...
    tvhdebug(LS_CAPMT2, "%s: CA_SET_DESCR_AES adapter %d par %d idx %d "
    ...
    if (parity == 0) {
      ...
      tvherror(LS_CAPMT2, "%s: Invalid parity %d in CA_SET_DESCR_AES for adapter%d", capmt_name(capmt), parity, adapter);
    ...
    tvhdebug(LS_CAPMT2, "%s: CA_SET_DESCR_MODE adapter %d index %d algo %d cipher mode %d",
    ...
    if (algo < 0 || algo > 2) {
      tvherror(LS_CAPMT2, "%s: Invalid algo %d", capmt_name(capmt), algo);
      return;
    }
    if (cipher_mode < 0 || cipher_mode > 1) {
      tvherror(LS_CAPMT2, "%s: Invalid cipher mode %d", capmt_name(capmt), cipher_mode);
      return;
    }
    ...
    tvhdebug(LS_CAPMT2, "%s: ECM_INFO: adapter=%d sid=%d caid=%04X(%s) pid=%04X provid=%06X ecmtime=%d hops=%d reader=%s from=%s protocol=%s",
    ...
    tvhinfo(LS_CAPMT2, "%s: Connected to server '%s' (protocol version %d)", capmt_name(capmt), info, protover);
    if (rev) {
    ...
    tvherror(LS_CAPMT2, "%s: unknown command %08X", capmt_name(capmt), cmd);
  }
}
  ...
  while (atomic_get(&capmt->capmt_running)) {
    ...
    for (i = 0; i < nfds; i++) {
      ...
      if (ev[i].ptr == &capmt->capmt_pipe) {
        ...
        tvhtrace(LS_CAPMT2, "%s: thread received shutdown", capmt_name(capmt));
      ...
      if (ret == 0) {
        tvhinfo(LS_CAPMT2, "%s: normal socket shutdown", capmt_name(capmt));
      ...
      tvhtrace(LS_CAPMT2, "%s: Received message from socket %i", capmt_name(capmt), recvsock);
      tvhlog_hexdump(LS_CAPMT2, buf, ret);
...
handle_ca0_wrapper(capmt_t *capmt)
{
  ...
  while (atomic_get(&capmt->capmt_running)) {
    ...
    if (ret < 0) {
      tvherror(LS_CAPMT2, "%s: error receiving over socket", capmt_name(capmt));
      break;
    } else if (ret == 0) {
      ...
      tvhtrace(LS_CAPMT2, "%s: Received message from socket %i", capmt_name(capmt), capmt->capmt_adapters[0].ca_sock);
      tvhlog_hexdump(LS_CAPMT2, buffer, ret);
  ...
  tvhinfo(LS_CAPMT2, "%s: connection from client closed ...", capmt_name(capmt));
}
#endif
#endif
...
capmt_create_udp_socket(capmt_t *capmt, int *socket, int port)
{
  *socket = tvh_socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
  if (*socket < 0) {
    tvherror(LS_CAPMT2, "%s: failed to create UDP socket", capmt_name(capmt));
    return 0;
  }
  ...
  {
    tvherror(LS_CAPMT2, "%s: failed to bind to ca0 (port %d)", capmt_name(capmt), port);
    return 0;
  }
  else
    return 1;
...
capmt_thread(void *aux) 
{
  ...
  tvhinfo(LS_CAPMT2, "%s active", capmt_name(capmt));
  ...
  while (atomic_get(&capmt->capmt_running)) {
    ...
    if (capmt->capmt_sock[0] >= 0) {
      ...
      if (capmt_oscam_new(capmt)) {
...
#endif
        {
          ...
          for (i = 0; i < is->is_count; i++) {
            ...
            n = la->la_dvb_number;
            if (n < 0 || n >= MAX_CA) {
              tvherror(LS_CAPMT2, "%s: adapter number > MAX_CA", capmt_name(capmt));
              continue;
            }
            tvhinfo(LS_CAPMT2, "%s: created UDP socket %d", capmt_name(capmt), n);
            bind_ok = capmt_create_udp_socket(capmt,
     ...
     if (capmt_oscam_network(capmt) ||
       ...
       tvherror(LS_CAPMT2, "%s: Only modes 3,4,5,6 are supported for non-linuxdvb devices", capmt_name(capmt));
       fatal = 1;
     }
#endif
    }
    ...
    tvhinfo(LS_CAPMT2, "%s: Automatic reconnection attempt in %d seconds", capmt_name(capmt), d);
  ...
  tvhinfo(LS_CAPMT2, "%s inactive", capmt_name(capmt));
  return NULL;
}
...
capmt_send_request(capmt_service_t *ct, int lm)
{
  ...
  LIST_FOREACH(cce2, &ct->ct_caid_ecm, cce_link) {
    ...
    if (cce2->cce_providerid) { // we need to add provider ID to the data
      if (cce2->cce_caid >> 8 == 0x01) {
        ...
        tvhwarn(LS_CAPMT2, "%s: Unknown CAID type, don't know where to put provider ID", capmt_name(capmt));
      }
    }
    pos = pos2;
    tvhdebug(LS_CAPMT2, "%s: adding ECMPID=0x%X (%d), "
  ...
  if(ct->td_keystate != DS_RESOLVED)
    tvhdebug(LS_CAPMT2, "%s: Trying to obtain key for service \"%s\"",
             capmt_name(capmt), t->s_dvb_svcname);
...
capmt_enumerate_services(capmt_t *capmt, int force)
{
  ...
  if (!all_srv_count) {
    // closing socket (oscam handle this as event and stop decrypting)
    tvhdebug(LS_CAPMT2, "%s: %s: no subscribed services, closing socket, fd=%d",
             capmt_name(capmt), __FUNCTION__, capmt->capmt_sock[0]);
    capmt->capmt_list_sent = 0;
    if (capmt->capmt_sock[0] >= 0)
      caclient_set_status((caclient_t *)capmt, CACLIENT_STATUS_READY);
    if (capmt_oscam_netproto(capmt)) {
    ...
    /* fresh connection or forced refresh: (re)send the full list */
    LIST_FOREACH(ct, &capmt->capmt_services, ct_link) {
    ...
    capmt->capmt_list_sent = 1;
  } else if (res_srv_count != all_srv_count) {
    if (!capmt->capmt_list_sent) {
      /* no full list sent yet on this connection - do it now */
      LIST_FOREACH(ct, &capmt->capmt_services, ct_link) {
        if (all_srv_count == i + 1)
          lm |= CAPMT_LIST_LAST;
        capmt_send_request(ct, lm);
      ...
      /* incremental CAPMT: ADD only the newly attached, not-yet-resolved
       * service(s); leave already-descrambling services on this same
       * connection completely untouched so starting one more channel
       * does not disturb the ones already running */
      LIST_FOREACH(ct, &capmt->capmt_services, ct_link) {
        if (ct->td_keystate != DS_RESOLVED) {
          tvhdebug(LS_CAPMT2, "%s: %s: incremental ADD for service \"%s\"",
                   capmt_name(capmt), __FUNCTION__,
                   ((mpegts_service_t *)ct->td_service)->s_dvb_svcname);
          capmt_send_request(ct, CAPMT_LIST_ADD);
        }
      }
    }
  }
...
{
  capmt_t *capmt = (capmt_t *)cac;
  tvhinfo(LS_CAPMT2, "%s: mode %i %s %s port %i destroyed",
          capmt_name(capmt),
          capmt->capmt_oscam,
...
capmt_conf_changed(caclient_t *cac)
{
  ...
  if (val2str(capmt->capmt_oscam, caclient_capmt_oscam_mode_tab) == NULL) {
    tvherror(LS_CAPMT2, "Unknown mode %d, disabling capmt client %s",
...
const idclass_t caclient_capmt2_class =
{
  .ic_super      = &caclient_class,
  .ic_class      = "caclient_capmt2",
  .ic_caption    = N_("CAPMT2 (OSCam Network, incremental list mgmt)"),
